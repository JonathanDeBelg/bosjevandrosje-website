  ...
  */
  public function confirmForm(array &$form, FormStateInterface $form_state, WebformSubmissionInterface $webform_submission) {
    ...
    $customerData = [
      ...
      'shipping_address' => [
        'street' => $data['adres'] . ' ' . $data['huisnummer'],
        'city' => json_decode($data['woonplaats'], TRUE)['value'], // Extract city
        'zip' => $data['postcode'],
        'country' => 'Netherlands',
      ],
      'mobile' => $data['mobiele_telefoonnummer'],
    ...
    try {
      ...
      $cleanDate = trim($dateParts[0]); // e.g., "7-02-2025"
      ...
      if ($isCashPayment) {
        ...
        \Drupal::logger('zoho_webhook')->info('Subscription data prepared for online payment: @data', [
        ...
        if (isset($response['hostedpage']['url'])) {
          ...
          \Drupal::logger('zoho_webhook')->info('Subscription not created. Response: @data', [
  ...
  private function getPlanCode($frequentie, $pakketkeuze) {
    ...
    $pakketkeuze = strtolower($pakketkeuze);
      ...
      if ($frequentie == 4) {
          return "{$pakketkeuze}_weekly"; // M_week, L_week, XL_week
      } elseif ($frequentie == 2) {
